...
def run_optimization(daily_load, load_price, charge_price, discharge_price, export_price,
    ...
    #                 cp.minimum(battery_power_kW, load) - cp.maximum(cp.minimum(battery_power_kW, load), 0)]
    # constraints += [0 <= battery_discharge_to_grid,
    #                 battery_discharge_to_grid <= battery_power_kW - cp.maximum(battery_power_kW, 0)]
    # constraints += [battery_discharge_to_load + battery_discharge_to_grid <= battery_power_kW]
    # constraints += [cp.maximum(- battery_power_kW, 0) <= battery_charge, battery_charge <= 0]
...
def run_mpc(raw_data, battery_size_kWh, battery_power_kW, min_soc, max_soc, one_way_efficiency, demand_charge,
    ...
    results["is_exporting_mpc"] = is_exporting_mpc
    results["is_charging_mpc"] = is_charging_mpc
...
# Specify prices
raw_data["load_price"] = raw_data["DA Price ($/kWh)"]
raw_data["charge_price"] = raw_data["DA Price ($/kWh)"]
raw_data["discharge_price"] = raw_data["DA Price ($/kWh)"]
raw_data["export_price"] = 0.03
